...
from stockWeb.models import Stock_articles,Stock_data,Stock_code
...
def stock_data(request):
    ...
    #qs = cursor.execute('SELECT sd.code,sc.name,publish_date,trading_volume,transaction,turn_over,opening_price,highest_price,lowest_price,closing_price,change,last_bid_price,last_ask_price,price_earnings_ratio FROM "stockWeb_stock_data" sd LEFT JOIN "stockWeb_stock_code" sc ON sd.code=sc.code WHERE publish_date>=%s LIMIT 50',[datetime.today().strftime('%Y-%m-%d')])
